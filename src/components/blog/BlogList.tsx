  ...
  return (
    <div>
      <div className="flex flex-col divide-y divide-[var(--color-border)]">
        {visible.map((art) => (
          <a
            key={art.id}
            href={`/blog/${art.id}`}
            className="group flex items-center bg-white hover:bg-[var(--color-calcs-bg)] border-l-2 border-transparent hover:border-[var(--color-accent)] transition-all duration-150 overflow-hidden"
          >
            <div className="flex-1 min-w-0 px-5 py-5">
              <div className="flex items-center gap-2.5 mb-2 flex-wrap">
                {art.categoria && (
                  <span className="text-[10px] font-semibold uppercase tracking-widest bg-[var(--color-accent-bg)] text-[var(--color-accent)] px-2 py-0.5 rounded-full">
                    {CATEGORIA_LABEL[art.categoria] ?? art.categoria}
                  </span>
                )}
                <span className="text-[11px] text-[var(--color-text-muted)]">
                  {new Date(art.fecha + 'T00:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
                </span>
              </div>
              <p className="text-base font-semibold text-[var(--color-text)] mb-1 leading-snug group-hover:text-[var(--color-accent)] transition-colors">
                {art.titulo}
              </p>
              {art.descripcion && (
      ...
      <p className="text-center text-xs text-[var(--color-text-muted)] mt-4">
        {articles.length} {articles.length === 1 ? 'artículo' : 'artículos'} · página {page} de {totalPages}
      </p>
    </div>
  );
}
