...
type Modo = 'tension' | 'intensidad' | 'resistencia';

export default function LeyOhmTool() {
  const [modo, setModo] = useState<Modo>('tension');
  ...
  const modos: { value: Modo; label: string; labelA: string; labelB: string; unitR: string } [] = [
    { value: 'tension',     label: 'Calcular tensión (V)',     labelA: 'Intensidad (A)', labelB: 'Resistencia (Ω)', unitR: 'V' },
    { value: 'intensidad',  label: 'Calcular intensidad (A)',  labelA: 'Tensión (V)',    labelB: 'Resistencia (Ω)', unitR: 'A' },
    { value: 'resistencia', label: 'Calcular resistencia (Ω)', labelA: 'Tensión (V)',    labelB: 'Intensidad (A)', unitR: 'Ω' },
  ];
  ...
  function calcular() {
    try {
      ...
      if (modo === 'resistencia') setResult(na / nb);
    } catch (e) {
  ...
  return (
    <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 space-y-4">
      <p className="text-xs text-[var(--color-text-secondary)] font-mono">V = I · R</p>
      ...
      {result !== null && (
        <div className="rounded-xl bg-[var(--color-calcs-bg)] p-4">
          <p className="text-2xl font-extrabold text-[var(--color-text)]">{formatNumber(result, 6)} {m.unitR}</p>
        </div>
      )}
    </div>
  );
